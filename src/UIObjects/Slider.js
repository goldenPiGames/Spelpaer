function Slider(x, y, width, height, text, hoverText, min, max, handler, getter) {
	this.x = x;
	this.y = y;
	this.width = width;
	this.height = height;
...
Slider.prototype.draw = function() {
	...
	ctx.textAlign = "right";
	ctx.strokeText(val, this.x+this.width-3, this.y+3);
	ctx.fillText(val, this.x+this.width-3, this.y+3);
}

// ------------------------------------------------------------------------------ Color Picker ---------------------------------

function ColorPicker(x, y, text, hoverText, handler, initial) {
	var thisser = this;
	this.x = x;
	this.y = y;
	this.width = 255;
	...
	this.redSlider =   new Slider(this.x, this.y+ 65, 255, 25, "Red", "The amount of red that goes into the color. Mix it with blue to get magenta, or with green to get yellow.", 0, 255, function(val){thisser.changed=true;thisser.red=val}, function(){return thisser.red});
	this.redSlider.forceColor = "#FF0000"; this.redSlider.forceBG = "#000000";
	this.greenSlider = new Slider(this.x, this.y+ 95, 255, 25, "Green", "The amount of green that goes into the color. Mix it with red to get yellow, or with blue to get cyan.", 0, 255, function(val){thisser.changed=true;thisser.green=val}, function(){return thisser.green});
	this.greenSlider.forceColor = "#00FF00"; this.greenSlider.forceBG = "#000000";
	this.blueSlider =  new Slider(this.x, this.y+125, 255, 25, "Blue", "The amount of blue that goes into the color. Mix it with green to get cyan, or with red to get magenta.", 0, 255, function(val){thisser.changed=true;thisser.blue=val}, function(){return thisser.blue});
...
ColorPicker.prototype = Object.create(UIObjectBase);
